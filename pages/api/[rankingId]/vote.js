...
async function handler(req, res) {
  ...
  if (req.body.winnerId === undefined) return res.status(400).json({ msg: "Missing Winning Vote" }); // 400 Bad Request
  if (req.body.loserId === undefined) return res.status(400).json({ msg: "Missing Losing Vote" }); // 400 Bad Request
  if (req.body.loserId < 0 || req.body.winnerId < 0) return res.status(400).json({ msg: "Invalid Vote" }); // 400 Bad Request

  // UPDATE query to the database, one for each vote
  // For winning vote: MatchupsWon++, matchups++;
  // For losing vote: matchups++;
  await query(db, "UPDATE Choice SET matchups = matchups + 1 WHERE ranking = $rankingId AND id = $choiceId", {
    $rankingId: rankingId,
  ...
  res.status(200).json({ msg: `Successfully Voted For ${req.body.winnerId}`, rankingId: rankingId }); // 200 Created
}
